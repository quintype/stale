...
module.exports = class Stale {
  ...
  postToSlackImpl(issues) {

  }

  async postToSlack(issues) {
    if (shouldPostToSlack(type)) {
      const timeToPost = this.getConfigValue(type, 'timeToPost')
      const currentTime = getHHMM(new Date())
      let diffTimeInMs = (timeToPost - currentTime) * 60 * 1000
      setTimeout(() => postToSlackImpl(issues), diffTimeInMs);
    }
  }
