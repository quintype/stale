...
function getHHMM(date) {
  ...
  return parseInt(hour+''+minutes, 10);
}
...
module.exports = class Stale {
  ...
  shouldPostToSlack(type) {
    const timeToPost = parseInt(this.getConfigValue(type, 'timeToPost'), 10)
    const currentTime = getHHMM(new Date())
    const timeAfterAnHour = currentTime + 100

    return currentTime <= timeToPost && timeAfterAnHour >= timeToPost
  }

  formatIssues(issues) {
    return issues.map(issue => {
      let age = dateFns.differenceInDays(new Date(), issue.updated_at)
      return `<${issue.html_url}|${issue.title}> - stale for *${age} days*.`
    });
  }

  async postToSlackImpl(type, issues) {
    const text = this.formatIssues(issues).join('\n')
    const url = this.getConfigValue(type, 'slackWebhookUrl')
    await request({
      url: url,
      method: 'POST',
      body: {
  ...
  getTimeToPostFromNowMs(type) {
    const timeToPost = this.getConfigValue(type, 'timeToPost')
    const hourToPost = Math.round(timeToPost / 100)
    const minuteToPost = timeToPost % 100
    const timestampToPost = new Date()
    timestampToPost.setHours(hourToPost)
    timestampToPost.setMinutes(minuteToPost)
    timestampToPost.setSeconds(0)
    const currentTime = new Date()
    return dateFns.differenceInMilliseconds(timestampToPost, currentTime);
  }

  async postToSlack(type, issues) {
    if (issues.length && this.shouldPostToSlack(type)) {
      setTimeout(() => this.postToSlackImpl(type, issues), this.getTimeToPostFromNowMs(type));
      this.logger.info({}, `scheduled to post to slack in ${this.getTimeToPostFromNowMs(type)}ms`);
    }
  }
  ...
  async markAndSweep (type) {
    ...
    if (this.getConfigValue(type, 'staleLabel')) {
      await this.ensureStaleLabelExists(type)
    }

    this.getStale(type).then(res => {
      let issues = res.data.items.filter(issue => !issue.locked)
      if (this.getConfigValue(type, 'staleLabel')) {
        issues.forEach(issue => this.mark(type, issue))
      }
      if (this.getConfigValue(type, 'timeToPost') && type === 'issues') {
        this.postToSlack(type, issues)
      }
    })
  ...
  search (type, days, query) {
    ...
    const params = {q: query, sort: 'updated', order: 'asc', per_page: maxActionsPerRun}
  ...
  async mark (type, issue) {
    ...
    if (perform) {
      ...
      await this.github.issues.addLabels({owner, repo, number, labels: [staleLabel]})
    } else {
