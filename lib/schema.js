...
const fields = {
  ...
  timeToPost: Joi.number()
    .description('Time to post to slack everyday'),

  slackWebhookUrl: Joi.string()
    .description('Slack webhook URL to post stale issues list'),
  ...
  staleLabel: Joi.alternatives().try(Joi.string(), Joi.boolean().only(false))
    .description('Label to use when marking as stale. Set to `false` to disable'),
...
const schema = Joi.object().keys({
  ...
  timeToPost: fields.timeToPost,
  slackWebhookUrl: fields.slackWebhookUrl
})
